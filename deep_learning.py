...
class NeuralNet(nn.Module):
        def __init__(self, input_size, hidden_sizes, num_output):
            ...
            self.fcs = []
            self.fcs.append(nn.Linear(input_size, hidden_sizes[0]))  # input - hidden
            for i in range(nhiddens - 1):   # middle
                self.fcs.append(nn.Linear(hidden_sizes[i], hidden_sizes[i+1]))
            self.fcs.append(nn.Linear(hidden_sizes[-1], num_output)) # output hidden
            self.relu = nn.ReLU()
...
def test_model(model, test_loader, criterion, device):
    ...
    print(f"TEST RESULT: average MESloss = {np.average(test_losses)} ")

    return test_losses, predicted_spectral_list
...
train_loader, valid_loader, criterion,  train_idxs, valid_idxs, test_idxs, resume = False):
    ...
    # valid_loader = torch.utils.data.DataLoader(dataset=dataset_valid, shuffle=True)
    # test_loader = torch.utils.data.DataLoader(dataset=dataset_test, shuffle=False)

    # print_dic(model, optimizer)
